...
void Clock_12_Start(void) 
{
    ...
    Clock_12_CLKEN |= Clock_12_CLKEN_MASK;
	Clock_12_CLKSTBY |= Clock_12_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_12_Stop
********************************************************************************
*
* Summary:
...
void Clock_12_Stop(void) 
{
    ...
    Clock_12_CLKEN &= (uint8)(~Clock_12_CLKEN_MASK);
	Clock_12_CLKSTBY &= (uint8)(~Clock_12_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_12_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_12_CLKEN & Clock_12_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_12_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_12_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_12__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_12_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_12_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_12_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_12_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_12_GetSourceRegister();
    uint16 oldDivider = Clock_12_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_12_CLKEN & Clock_12_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_12_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_12_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_12_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_12_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_12_MOD_SRC |= modeBitMask & (uint8)Clock_12_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_12_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_12_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_12_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_12_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_12_MOD_SRC & (uint8)(Clock_12_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_12_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_12_GetDividerRegister();
    uint8 oldSrc = Clock_12_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_12_MOD_SRC |= CYCLK_SSS;
        Clock_12_MOD_SRC =
            (Clock_12_MOD_SRC & (uint8)(~Clock_12_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_12_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_12_MOD_SRC & Clock_12_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_12_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_12_PHASE = clkPhase & Clock_12_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_12_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_12_PHASE & Clock_12_PHASE_MASK;
}
