...
/*******************************************************************************
* Function Name: SPI_Clock_In_Start
********************************************************************************
*
* Summary:
...
void SPI_Clock_In_Start(void) 
{
    ...
    SPI_Clock_In_CLKEN |= SPI_Clock_In_CLKEN_MASK;
	SPI_Clock_In_CLKSTBY |= SPI_Clock_In_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SPI_Clock_In_Stop
********************************************************************************
*
* Summary:
...
void SPI_Clock_In_Stop(void) 
{
    ...
    SPI_Clock_In_CLKEN &= (uint8)(~SPI_Clock_In_CLKEN_MASK);
	SPI_Clock_In_CLKSTBY &= (uint8)(~SPI_Clock_In_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SPI_Clock_In_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SPI_Clock_In_CLKEN & SPI_Clock_In_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SPI_Clock_In_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SPI_Clock_In_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SPI_Clock_In__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SPI_Clock_In_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SPI_Clock_In_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SPI_Clock_In_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_Clock_In_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SPI_Clock_In_GetSourceRegister();
    uint16 oldDivider = SPI_Clock_In_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SPI_Clock_In_CLKEN & SPI_Clock_In_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SPI_Clock_In_DIV_PTR, clkDivider);
                SPI_Clock_In_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SPI_Clock_In_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SPI_Clock_In_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SPI_Clock_In_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SPI_Clock_In_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SPI_Clock_In_DIV_PTR);
}


/*******************************************************************************
* Function Name: SPI_Clock_In_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SPI_Clock_In_MOD_SRC |= modeBitMask & (uint8)SPI_Clock_In_MODE_MASK;
}


/*******************************************************************************
* Function Name: SPI_Clock_In_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SPI_Clock_In_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SPI_Clock_In_MODE_MASK));
}


/*******************************************************************************
* Function Name: SPI_Clock_In_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SPI_Clock_In_MOD_SRC & (uint8)(SPI_Clock_In_MODE_MASK);
}


/*******************************************************************************
* Function Name: SPI_Clock_In_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SPI_Clock_In_GetDividerRegister();
    uint8 oldSrc = SPI_Clock_In_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SPI_Clock_In_MOD_SRC |= CYCLK_SSS;
        SPI_Clock_In_MOD_SRC =
            (SPI_Clock_In_MOD_SRC & (uint8)(~SPI_Clock_In_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SPI_Clock_In_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SPI_Clock_In_MOD_SRC & SPI_Clock_In_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SPI_Clock_In_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SPI_Clock_In_PHASE = clkPhase & SPI_Clock_In_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SPI_Clock_In_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SPI_Clock_In_PHASE & SPI_Clock_In_PHASE_MASK;
}
