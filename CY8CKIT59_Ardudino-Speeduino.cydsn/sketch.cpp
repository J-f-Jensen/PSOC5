...
// Define 1ms test function
void onemsTest(void);

// Define the variable used in 1msTest
int onemsCounter = 0;

// Define the adress to place the EEPROM tests data in
#define EEPROM_TEST    2
...
void setup() {
  ...
  EEPROM.update(EEPROM_TEST, 'X');
  
  // 1ms counter test
  Counter_1ms_initialize();
  Counter_1ms_attachinterupt(onemsTest);
}
...
void loop() {
  ...
  if (Serial.available() > 0) 
  {
    ...
    switch ( incomingByte ) {
        case 'H': // if it's a capital H (ASCII 72), turn on the LED:
                digitalWrite(ledPin, HIGH);
                Serial.println("HIGH");
          break;
          
        case 'L': // if it's an capital L (ASCII 76) turn off the LED:
                digitalWrite(ledPin, LOW);
                Serial.println("LOW");
          break;
        
        case 'E': // if it's an capital E (ASCII ) then we test the EEPROM:
                Serial.write("Old EEPROM DATA");
                Serial.write(EEPROM.read(EEPROM_TEST));
                
                if (EEPROM.read(EEPROM_TEST) == 'X')
                    {
                        EEPROM.update(EEPROM_TEST, 'Y');
                    }
                else
                    {
                        EEPROM.update(EEPROM_TEST, 'X');
...
void onemsTest(void)
{
    ...
    if (onemsCounter == 1000)
    {
        // reset the counter value
        onemsCounter = 0;
        
        // flip the LED to show it works
