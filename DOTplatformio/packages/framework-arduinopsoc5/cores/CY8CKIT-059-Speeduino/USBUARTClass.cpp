...
uint8_t USBUARTClass::buffer( void )
{
  ...
  if (dataBufferSize > 0)
  {
    // We need to check if there is space in the buffer to recive data, if not then we have to wait until the data in the buffer is read
    if ((_appBuffSize + dataBufferSize) < 65)
    {
        dataBufferSize = USBUART_GetAll((_appBuffer+_appBuffSize));
...
size_t USBUARTClass::write( const uint8_t uc_data )
{
  ...
  // Check and wait until USB service is configured - If this not is checked it can lock the application
  ...
  /* Check and wait for the port to be ready */   
...
size_t USBUARTClass::_usb_serial_write( const uint8_t* buffer, uint16_t size )
{
  ...
  */
  ...
  {
    // Service 'USB CDC' if device is configured
